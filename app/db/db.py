...
class Database:
    ...
    def all(self, table_name) -> List[OrderedDict]:
    ...
    def filter(self, table_name: str, **kwargs) -> List[OrderedDict]:
        """sumary_line
            id=1 \n
            id__gt=4 ---> id > 4 \n
            id__gte=4 ---> id >= 4 \n
    ...
    def exists(self, table_name: str, **kwargs):
        self.cursor.execute(f"SELECT * FROM {table_name}")
        fetch = self.cursor.fetchall()
        for row in fetch:
    ...
    def update(self, table_name: str, key, key_value, **kwargs):
        """UPDATE weather SET
    ...
    def get_user_id(self, table_name: str, **kwargs):
    ...
    def query(self, query: str) -> List[OrderedDict]:
        self.cursor.execute(query)
